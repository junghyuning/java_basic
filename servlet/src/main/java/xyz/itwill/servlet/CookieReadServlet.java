...
public class CookieReadServlet extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response)
		...
		// 클라이언트에게 전달받은 모든 쿠키를 반환받아 저장함
		// HttpServletRequest.getCookie() : 클라이언트에게 전달받은 모든 쿠키를 얻어와 Cookie 객체 배열로 반환하는 메소드
		...
		if (cookies == null) { //클라이언트로부터 전달받은 쿠키가 없는 경우
			...
			String id = "";
			String count = "";
			// 쿠키 객체 배열의 요소를 차례로 제공받아 사용하기 위한 반복문
			...
			if (!count.equals("")) {
				...
				// => 전달되는 쿠키 객체의 쿠키명과 같은 쿠키가 이미 클라이언트에 저장돼있는경우 count 가 증가하지 않음
				// => 덮어 씌우기를 통해 쿠키를 변경하면 새로운 쿠키를 생성 -> count 증가
