...
//		Employee e = new Employee(); // Employee는 추상클래스이므로 객체 생성시, error!
		...
		for(Employee employee : empArray) {
			 ...
			 *
			 * //오버라이딩 하지않은 자식클래스의 메서드 사용하고 싶을 시, 형변환 해줘야 함. if(employee instanceof
			 * EmployeeRegular) { System.out.println("급여 = " +
			 * ((EmployeeRegular)employee).computeSalary()); }else if(employee instanceof
			 * EmployeePartTime) { System.out.println("급여 = " +
			 * ((EmployeePartTime)employee).computeTimePay());
			 * 
			 * }else if(employee instanceof EmployeeContract) { System.out.println("급여 = " +
			 * ((EmployeeContract)employee).computeContract());
			 * 
			 * }
			 */
		}
